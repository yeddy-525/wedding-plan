...
- 타임라인 카테고리 입력에 자동완성 추가 — `tagSuggestions()`가 **기타 업체의 `tags`(종류)만** + 업체 카테고리명(`CN` values) + 기존에 쓰인 타임라인 카테고리를 모아서 `<datalist id="tlt-cat-list">`로 제공. 할 일 추가 시 카테고리를 필수 입력으로 바꿈(이전엔 항상 '기타'로 자동 지정되어 나중에 수정해서 넣어야 했음) — 특정 업체와 무관한 일반 항목은 `전체`를 입력하면 됨
  - **주의**: 처음엔 실수로 모든 카테고리(`Object.values(D.vendors).flat()`)의 `tags`를 다 긁어왔었음 — 웨딩홀/스튜디오/드레스/메이크업의 특징 태그(예: "서울숲촬영가능")까지 섞여 나오는 버그였고, 2026-07-31에 기타만 보게 수정함
...
- **타임라인** — 결혼 전 개월수 기준 체크리스트. 각 구간 제목("결혼 10개월 전" 등) 옆에 실제 연월("2026년 11월")을 계산해서 같이 표시 — `mb`(결혼 전 개월수)만 봐서는 실제 언제인지 감이 안 온다는 피드백으로 추가 (2026-07-31)
...
### 타임라인 클릭하면 제이웨딩 화면 그대로 남음 (2026-07-31 수정)

**증상**: 사이드바에서 "타임라인" 클릭 시 상단 타이틀은 "타임라인"으로 바뀌는데, 본문은 이전에 보고 있던 "제이웨딩" 화면 그대로 남아있음

**원인**: `tagSuggestions()`에서 `const s=new Set()` 다음 줄에 세미콜론 없이 `(D.vendors.etc||[]).forEach(...)`를 바로 이어씀 — 자바스크립트 자동 세미콜론 삽입(ASI) 규칙 때문에 `new Set()(D.vendors.etc||[])`로 파싱되어 "Set 인스턴스를 함수처럼 호출"하는 걸로 해석되고, `TypeError: ... is not a function`이 던져짐. `go()`는 타이틀을 `render()` 호출 전에 먼저 세팅하기 때문에 타이틀만 바뀌고, `render()` 안에서 `rTl()`이 이 에러로 죽으면서 `el.innerHTML`이 갱신 안 되고 이전 화면이 그대로 남음. `new Function()` 기반 정적 문법 체크로는 안 잡히고 실제로 실행해봐야 드러나는 런타임 에러라 커밋 전에 못 걸러짐

**수정**: `;(D.vendors.etc||[]).forEach(...)`로 앞에 세미콜론 추가 — 이 코드베이스는 원래 세미콜론 없이 줄바꿈하는 스타일이라, `(`나 `[`로 시작하는 줄 앞에는 관례적으로 `;`를 붙임 (다른 곳들도 이미 이 패턴 사용 중)
