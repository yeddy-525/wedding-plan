...
- **타임라인** — 결혼 전 개월수 기준 체크리스트, **매달** 하나씩 구간이 있음(12,11,10...1,1주일 전 — 2026-07-31에 2개월 간격에서 매달로 세분화). 각 구간 제목("결혼 10개월 전" 등) 옆에 실제 연월("2026년 11월")을 계산해서 같이 표시 — `mb`(결혼 전 개월수)만 봐서는 실제 언제인지 감이 안 온다는 피드백으로 추가 (2026-07-31)
  - **세분화 이유**: 원래 2개월 간격(12,10,8...)이라 "드레스투어를 12월에 하고 싶은데 표시되는 달은 11월"처럼 특정 달에 하고 싶은 일이 애매하게 낄 때가 있었음. 항목마다 날짜를 따로 갖는 방식도 고려했으나, 구조 변경이 훨씬 크고 Code.gs도 손대야 해서, 대신 구간 자체를 매달 단위로 촘촘하게 만드는 쪽으로 감 — 항목 구조(`text`/`cat`/`dn`)는 그대로라 Code.gs 변경 불필요
  - `ensureTlMonths()` — `TL_MONTHS`(이제 `[12,11,...,1,0.25]`) 기준으로 빠진 달의 구간을 자동으로 채워 넣음(기존 구간/할 일은 그대로 유지, id는 새로 부여, `mb` 내림차순 정렬). 앱 초기 로드 시점과 `applyRemote()`(GAS 데이터 적용 시점) 둘 다에서 호출돼서, 기존에 8개 구간만 있던 사용자 데이터도 다음 로드 때 자동으로 13개 구간으로 채워짐 — 사용자가 뭘 안 해도 마이그레이션됨
  - 겸사겸사 발견: 앱 초기 로드 경로(`ld()` 직후)에는 `normalizeVendors()` 호출이 빠져있었음(`applyRemote()`에만 있었음) — 로컬 데이터만 쓰고 GAS 적용이 한 번도 안 된 상태에서는 업체 날짜 복구가 누락될 수 있었음. 이번에 같이 추가함
